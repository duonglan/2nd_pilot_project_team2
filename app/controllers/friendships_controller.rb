class FriendshipsController < ApplicationController
  ...
  def create
    ...
    @friendship1 = Friendship.create(user_id: @user.id, 
      friend_id: @friend.id, status: "requested")
    @friendship2 = Friendship.create(user_id: @friend.id, 
  ...
  def update
    ...
    params[:friendship1] = {user_id: @user.id, 
      friend_id: @friend.id, status: "accepted"}
    params[:friendship2] = {user_id: @friend.id, 
      friend_id: @user.id, status: "accepted"}
    @friendship1 = Friendship.find_by_user_id_and_friend_id(@user.id, 
      @friend.id)
    @friendship2 = Friendship.find_by_user_id_and_friend_id(@friend.id, 
      @user.id)
